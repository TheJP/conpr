...
object ScalaFutures extends App {
  ...
  val futureContent: Future[String] = 
    Future { loadURL("http://www.akka.io") }
  ...
  futureIndex.onSuccess{ 
    case m => println("Result: " + m) 
  } 
  
  futureIndex.onFailure{ 
    case t: UnknownHostException => println("Host not found: " + t.getMessage()) 
    case t: Exception => println("Unknown Problem: " + t.getMessage()) 
  } 
